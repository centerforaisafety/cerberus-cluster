...
function slurm_job_submit(job_desc, part_list, submit_uid)
    ...
    slurm.log_info(
        "job_submit.lua: uid=%d is_batch=%s part=%s gres=%s gpus=%d timelimit=%d",
        submit_uid,
        tostring(is_batch),
        partition,
        gres,
    ...
    if not is_batch then
        ...
        if gpu_count > max_total_gpus then
            ...
            if capped_gres == gres then
                capped_gres = "gpu:" .. max_total_gpus
            end
        ...
        slurm.log_info(
            "job_submit.lua: INTERACTIVE policy applied uid=%d gpus=%d time=%d priority=%d (base=%d + boost=%d)",
            submit_uid,
            gpu_count,
